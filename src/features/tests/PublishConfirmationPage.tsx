...
export const PublishConfirmationPage: React.FC = () => {
  ...
  const totalQuestions = parseInt(testDetails?.total_questions || testDetails?.rawForm?.noOfQuestions || '0', 10);
  ...
  const questions = Array.from({ length: totalQuestions }, (_, i) => ({ id: i + 1, text: `Question ${i + 1}` }));
  ...
  const bulkCreateMutation = useMutation({ mutationFn: questionApi.bulkCreate });
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (!questionsAlreadySent) {
        const questionsPayload = Object.values(testRecord.questions).map((q: any) => ({
          type: 'mcq',
        ...
        const bulkResult = await bulkCreateMutation.mutateAsync(questionsPayload);
        const createdQuestionIds = (bulkResult?.data || []).map((q: any) => q.id).filter(Boolean);
      ...
      await publishTestMutation.mutateAsync({ id: activeTestId, payload: publishPayload });
      ...
      setTimeout(() => {
        finalizePublish(activeTestId, { publishType, scheduleDate, scheduleTime, liveUntil }, finalStatus === 'live' ? 'Live' : 'Scheduled');
      }, 500);
      ...
      if (res?.errors) {
        if (Array.isArray(res.errors)) errMsg = res.errors.map((e: any) => e.msg || e.message).filter(Boolean).join('. ') || res.message || errMsg;
        else if (typeof res.errors === 'object') errMsg = res.errors.message || res.errors.msg || res.message || errMsg;
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <div className="bg-surface rounded-card shadow-2xl p-8 max-w-md w-full mx-4 text-center animate-in zoom-in-95 fade-in duration-300">
          <div className="w-16 h-16 mx-auto mb-5 rounded-full bg-green-100 flex items-center justify-center"><CheckCircle2 className="w-9 h-9 text-green-600" /></div>
          <h2 className="text-xl font-bold text-text-title mb-2">{publishType === 'now' ? 'Test Published!' : 'Test Scheduled!'} 🎉</h2>
          <p className="text-sm text-text-muted mb-2">{successMessage}</p>
          <p className="text-[13px] text-text-muted font-medium mb-6">"{publishedTestName}" — {questionsFilled || 'all'} questions</p>
          <div className="flex items-center justify-center gap-2 text-xs text-text-muted"><Loader2 className="w-3 h-3 animate-spin" /><span>Redirecting to dashboard...</span></div>
          <button onClick={() => navigate('/dashboard')} className="mt-5 px-8 py-2 text-sm font-medium text-brand-primary hover:bg-blue-50 rounded-subtle transition-colors">Go to Dashboard Now</button>
        </div>
      </div>
    );
  }
        ...
        <div className="bg-surface rounded-card shadow-2xl p-6 max-w-md w-full mx-4 animate-in zoom-in-95 fade-in duration-200">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-base font-bold text-text-title">Publish Failed</h3>
              <p className="text-sm text-text-muted mt-1.5 leading-relaxed">{publishError}</p>
            </div>
          </div>
          <div className="flex justify-end mt-5">
            <button onClick={() => setPublishError(null)} className="px-6 py-2 text-sm font-semibold text-white bg-brand-primary hover:bg-brand-hover rounded-subtle transition-colors shadow-sm">Got it</button>
          </div>
        </div>
      </div>
    );
  ...
  if (!testRecord) {
    return <div className="p-8 text-center text-text-muted">No active test found. Return to dashboard.</div>;
  }

  const isPublishing = bulkCreateMutation.isPending || publishTestMutation.isPending;
      ...
      <div className="flex-1 overflow-y-auto p-8">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="bg-surface border border-border-subtle rounded-card p-5 shadow-sm relative">
            <button onClick={() => navigate('/tests/new/questions')} className="absolute right-4 top-4 text-brand-primary hover:bg-blue-50 p-1.5 rounded transition-colors"><Edit3 className="w-4 h-4" /></button>
            ...
            <div className="grid grid-cols-2 gap-4 text-[13px]">
              <div className="space-y-4">
                <div className="grid grid-cols-[80px_1fr] items-center">
                  ...
                  <span className="font-medium text-text-title">: {displaySubject}</span>
                </div>
                <div className="grid grid-cols-[80px_1fr] items-center gap-1">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {testTopicArray?.map((topicId: string) => {
                      ...
                      return <span key={topicId} className="px-2 py-0.5 text-nowrap text-[11px] font-medium text-orange-600 bg-orange-50 border border-orange-200 rounded">{topicName}</span>;
                    })}
                  </div>
                </div>
                <div className="grid grid-cols-[80px_1fr] items-center gap-1">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {testSubTopicArray?.map((stId: string) => {
                      const stName = subTopics?.find((st: any) => st.id === stId)?.name;
                      if (!stName) return null;
                      return <span key={stId} className="px-2 py-0.5 text-nowrap text-[11px] font-medium text-yellow-600 bg-yellow-50 border border-yellow-200 rounded">{stName}</span>;
                    })}
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-2">
            <h2 className="text-lg font-bold text-text-title">
              Question {selectedQuestionId} <span className="text-brand-primary text-[13px] font-medium">/ {totalQuestions}</span>
            </h2>
            ...
            {selectedQuestion ? (
              <div className="bg-surface border border-border-subtle rounded-card p-6 shadow-sm space-y-5">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-text-title leading-relaxed" dangerouslySetInnerHTML={{ __html: selectedQuestion.questionText || 'No question text' }} />
                  </div>
                ...
                {selectedQuestion.mediaUrl && (
                  <div className="pl-11">
                    <div className="flex items-center gap-2 text-xs text-text-muted mb-2">
                      ...
                      <a href={selectedQuestion.mediaUrl} target="_blank" rel="noopener noreferrer" className="text-brand-primary hover:underline truncate max-w-md">{selectedQuestion.mediaUrl}</a>
                    </div>
                    <img
                  ...
                  {['option1', 'option2', 'option3', 'option4'].map((optKey) => {
                    ...
                    return (
                      <div key={optKey} className={`flex items-center gap-2.5 px-4 py-3 rounded-subtle text-sm border transition-all ${
                        ...
                        <span className="flex-1">{selectedQuestion[optKey] || '-'}</span>
                ...
                {selectedQuestion.solutionText && (
                  <div className="pl-11 pt-2 border-t border-border-subtle">
                    <p className="text-[13px] text-text-body">
                      <span className="font-semibold text-text-title">💡 Explanation:</span> {selectedQuestion.solutionText}
                    </p>
                  </div>
                )}

                {/* Question-level topic/subtopic if different from test level */}
                {(selectedQuestion.topic?.length > 0 || selectedQuestion.subTopic?.length > 0) && (
                  <div className="pl-11 pt-2 border-t border-border-subtle flex flex-wrap gap-2">
                    {selectedQuestion.topic?.map((tId: string) => {
                      const tName = topics?.find((t: any) => t.id === tId)?.name;
                      return tName ? <span key={tId} className="px-2 py-0.5 text-[10px] font-medium text-orange-600 bg-orange-50 border border-orange-200 rounded">{tName}</span> : null;
                    })}
                    {selectedQuestion.subTopic?.map((stId: string) => {
                      const stName = subTopics?.find((st: any) => st.id === stId)?.name;
                      return stName ? <span key={stId} className="px-2 py-0.5 text-[10px] font-medium text-yellow-600 bg-yellow-50 border border-yellow-200 rounded">{stName}</span> : null;
                    })}
                  </div>
                )}
              </div>
              ...
              <div className="bg-bg-main border border-border-subtle rounded-card p-8 text-center">
                <p className="text-sm text-text-muted">This question hasn't been filled yet.</p>
              </div>
            )}
          </div>
          ...
          <div className="pt-6 border-t border-border-subtle space-y-6">
            <div className="inline-flex p-1 bg-gray-50 border border-border-subtle rounded-md">
              <button onClick={() => setPublishType('now')} className={`px-6 py-2.5 text-sm font-semibold rounded transition-all ${publishType === 'now' ? 'bg-white text-text-title shadow-sm border border-gray-200' : 'text-text-muted hover:text-text-title border border-transparent bg-transparent'}`}>Publish Now</button>
              <button onClick={() => setPublishType('schedule')} className={`px-6 py-2.5 text-sm font-semibold rounded transition-all ${publishType === 'schedule' ? 'bg-white text-text-title shadow-sm border border-gray-200' : 'text-text-muted hover:text-text-title border border-transparent bg-transparent'}`}>Schedule Publish</button>
            </div>

            {publishType === 'schedule' && (
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="block text-[13px] font-semibold text-text-title">Select Date</label>
                  <input type="date" value={scheduleDate} onChange={(e) => setScheduleDate(e.target.value)} className="w-full px-4 py-2.5 bg-surface border border-border-subtle rounded-subtle text-sm outline-none focus:border-brand-primary" />
                </div>
                <div className="space-y-2">
                  <label className="block text-[13px] font-semibold text-text-title">Select Time</label>
                  <input type="time" value={scheduleTime} onChange={(e) => setScheduleTime(e.target.value)} className="w-full px-4 py-2.5 bg-surface border border-border-subtle rounded-subtle text-sm outline-none focus:border-brand-primary" />
                </div>
              </div>
            )}

            <div className="space-y-4 pt-6 border-t border-border-subtle">
              <h3 className="text-sm font-semibold text-text-title">Live Until</h3>
              <p className="text-[13px] text-text-muted">Choose how long this test should remain available.</p>
              <div className="grid grid-cols-2 gap-y-5 max-w-2xl pt-2">
                {[
                  ...
                  <label key={option.id} className="flex items-center gap-3 cursor-pointer">
                    <input type="radio" name="liveUntil" value={option.id} checked={liveUntil === option.id} onChange={() => setLiveUntil(option.id)} className="w-4 h-4 text-brand-primary border-gray-300 cursor-pointer" />
                    <span className="text-[13px] text-text-title">{option.label}</span>
                  </label>
                ))}
              </div>
              {liveUntil === 'custom' && (
                <div className="grid grid-cols-2 gap-6 mt-6 pt-6 border-t border-border-subtle">
                  <input type="date" value={customEndDate} onChange={(e) => setCustomEndDate(e.target.value)} className="w-full px-4 py-2.5 bg-surface border border-border-subtle rounded-subtle text-sm outline-none focus:border-brand-primary" />
                  <input type="time" value={customEndTime} onChange={(e) => setCustomEndTime(e.target.value)} className="w-full px-4 py-2.5 bg-surface border border-border-subtle rounded-subtle text-sm outline-none focus:border-brand-primary" />
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex justify-end gap-4 pt-8 border-t border-border-subtle">
            <button disabled={isPublishing} onClick={() => navigate('/tests/new/questions')} className="px-10 py-2.5 text-sm font-semibold text-brand-primary bg-blue-50/50 hover:bg-blue-50 rounded-subtle transition-colors disabled:opacity-50">Cancel</button>
            <button
              onClick={handleConfirm}
              disabled={(publishType === 'schedule' && (!scheduleDate || !scheduleTime)) || (liveUntil === 'custom' && (!customEndDate || !customEndTime)) || isPublishing}
